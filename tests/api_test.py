class TestApi:
    ...
    def test_api_post_1(self, test_client):
        ...
        assert type(response.json) == type({}), "Возвращается не словарь"
    ...
    def test_api_posts(self, test_client):
        ...
        assert type(response.json) == type([]), "Возвращается не список"

        posts_list = response.json
        for post in posts_list:
            assert type(post) == type({}), "Элемент списка не словарь"
